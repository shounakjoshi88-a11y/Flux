...
async function getEmbedding(text: string): Promise<number[]> {
    const model = (nim as any).embeddingModel(EMBED_MODEL);
    const { embedding } = await embed({ model, value: text } as any);
    return embedding as number[];
}

async function getRelevanceScore(query: string, passage: string): Promise<number> {
    const model = (nim as any).chatModel(RERANK_MODEL);
    const prompt = `Query: ${query}\nPassage: ${passage}\nHow relevant is the passage to the query on a scale of 0 to 1? Only output the number.`;
    const { text } = await generateText({ model, prompt, maxTokens: 5 } as any);
